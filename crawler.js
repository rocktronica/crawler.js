...
// do: more sensical settings argument passing, backbone cleanup!, ul.click !== cycle, nav listens to .running
// consider: page.parents(page1,page2), alternating iframes to avoid load flash, draggable panes

(function(exports, $){

	var SettingsOverride = SettingsOverride || exports.Settings, output = { default: true };
	var Settings = {
		title: "crawler.js",
		wait: 1000,
		deep: 5,
		...
		onFrameload: function(output, $output, $doc, page) {
			...
			var iWordCount = $doc.find("#main").text().trim().split(/\s+/).length;
	...
	var init = function(){
		...
		var sOriginalTitle = exports.document.title; // cache before reset
		$("link[rel='stylesheet']").not("#style").remove();
		var $body = $("body").empty().append("<link href='http://rocktronica.github.com/crawler.js/crawler.css' rel='stylesheet' id='style' type='text/css'>");
		...
		// have any global Settings?
		if (typeof SettingsOverride === "object") {
			settings = _.extend(settings, SettingsOverride);
		}
		exports.settings = settings;
	
		// i need variables before they're set. this is bad probably.
		var crawler; // set at end
		var frame; // set at 1st crawler cycle
		var header; // set at 
		...
		var Crawler = Backbone.Model.extend({
			...
			initialize: function() {
				exports.document.title =  settings.title + " " + settings.separator + " " + sOriginalTitle;
			...
			cycle: function() {
				...
				frame = frame || new Frame({ model: page });
				if (!page) {
					this.done();
					return false;
				}
